...
// multiple dimensions, because we can have dependent demand (packages, constraints)
...
struct demand_supply_function
{
    ...
    /// \param variables
    /// \return
    virtual std::vector<double> excess_demand(const std::vector<esl::economics::quote> &quotes,
...
: public virtual demand_supply_function
{
    ...
    virtual std::vector<esl::variable> excess_demand(const std::vector<esl::economics::quote> &quotes,  std::vector<esl::variable> &variables) const  = 0;

    ///
    /// \brief  Shim to convert from auto-differentiated variables to double
    ///
    virtual std::vector<double> excess_demand(const std::vector<esl::economics::quote> &quotes, const std::vector<double> &variables) const override
    {
        ...
        std::vector<esl::variable> intermediate_ = excess_demand(quotes, variables_);
...
///
/// \brief                  excess demand (demand-supply) function at the type level.
///
/// \tparam property_ts_    the different properties that are covered by this function (separate dimensions)
//template<typename ... property_ts_>
//struct demand_supply_function_t
//{
//    template<typename scalar_model_ = quantity>
//    array<scalar_model_, sizeof ... (property_ts_)> at(const array<quote, sizeof ... (property_ts_)> &quotes) = 0;
//};
